turtles-own
  [ sick?                ;; if true, the turtle is infectious
    ...
    nb-confinement       ;; nb aléatoire correspondant au respect du confinement si malade
    vaccine]             ;; la personne est vaccinée ou non

globals
  [ %infected            ;; what % of the population is infectious
    %immune              ;; what % of the population is immune
    nb-dead              ;; nombre de morts du virus
    nb-people-total      ;; nombre total de personnes ayant vécue
    nb-turtles
    nb-vaccines          ;; nombre de personnes vaccinées
...
to setup-turtles
  create-turtles number-people
    [ setxy random-xcor random-ycor
      ...
      get-healthy
      set nb-people-total number-people
      set nb-confinement random-float 100
      set vaccine 0]
  ask n-of malades-depart turtles
    [ get-sick ]
  ask n-of vaccines-depart turtles
    [ set vaccine 1 ]
end
...
to setup-constants
  ...
  if immunite-a-vie [ set immunity-duration 2600 ]
  if not immunite-a-vie [ set immunity-duration weeks-immunite ]
end
...
to update-global-variables
  if count turtles > 0
    [ set %infected (count turtles with [ sick? ] / count turtles) * 100
      ...
      set nb-turtles (count turtles)

  ]

  if ticks mod 7 = 0
    [ ask n-of vaccines-par-7-semaines turtles
      [ set vaccine 1 ] ]
...
to move ;; turtle procedure
  if malades-confines
    [ ifelse sick?
      [ ifelse nb-confinement > %confines
        [ rt random 100
          lt random 100
          fd 1 ]
        [ fd 0 ] ]
...
to reproduce
  if count turtles < carrying-capacity and random-float 100 < chance-reproduce
    [ set nb-people-total nb-people-total + 1
      hatch 1
      [ set age 1
        lt 45 fd 1
        get-healthy ] ]
